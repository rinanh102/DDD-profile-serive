...
import { IGetListProfilesUseCase, IGetProfileUseCase } from '@cbidigital/profile-module/features/profile/app/usecases/profile/query';
import {
    Body,
    CacheLookup,
    CacheStore,
...
@Rest('/admin/profiles/laity')
export class ProfileLaityRest {
    constructor(
        @CacheLookup() readonly cacheStore: CacheStore,
        @Inject(ProfileCatholicTokens.Usecase.CREATE_PROFILE_LAITY)
        private readonly _createProfileLaityUseCase: ICreateProfileUseCase,
        @Inject(ProfileCatholicTokens.Usecase.UPDATE_PROFILE_LAITY)
        private readonly _updateProfileLaityUseCase: IUpdateprofileUseCase,
    ...
    public async create(@Body() body: CreateProfileUseCaseInput, @Principal('sub') authId: string) {
        body.userId = null;
        return this._createProfileLaityUseCase.exec(body, {
            auth: {
                authId,
            },
        });
    ...
    public async update(@Param('id') id: string, @Body() body: any, @Principal('sub') authId: string) {
        return this._updateProfileLaityUseCase.exec(
            {
                ...body,
                id,
            },
    ...
    public async findAll(
        ...
        eavFilter[positionGroupCode] = { eq: PositionGroup.Laity };
